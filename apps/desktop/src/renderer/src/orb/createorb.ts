...
export default function createOrb(
	...
	let targetSpeed = 0.01, currentSpeed = 0.01;
	let targetBright = 0, currentBright = 0;
	let targetSize = 0.1, currentSize = 0.1;
	...
	let targetCamPosX = 0, targetCamPosY = 0;
	...
	function animate() {
		...
		function assignTargets(
			...
			if (window.__Params) {
				...
				if (window.__Params.timer >= window.__Params.interval || (lastState !== "startup" && state != lastState) || forced) {
					window.__Params.timer = 0;
					window.__Params.interval = 6 + Math.random() * 6;
					const { radius, speed, bright, size, lineAmount, electronRate, vortex, breathAmp } = data;
					targetRadius = typeof radius === "number" ? radius : ((radius.min + radius.max) / 2) + (Math.random() - 0.5) * (radius.max - radius.min);
					targetSpeed = typeof speed === "number" ? speed : ((speed.min + speed.max) / 2) + (Math.random() - 0.5) * (speed.max - speed.min);
					targetBright = typeof bright === "number" ? bright : ((bright.min + bright.max) / 2) + (Math.random() - 0.5) * (bright.max - bright.min);
					targetSize = typeof size === "number" ? size : ((size.min + size.max) / 2) + (Math.random() - 0.5) * (size.max - size.min);
					targetLineAmount = typeof lineAmount === "number" ? lineAmount : ((lineAmount.min + lineAmount.max) / 2) + (Math.random() - 0.5) * (lineAmount.max - lineAmount.min);
					targetElectronRate = typeof electronRate === "number" ? electronRate : ((electronRate.min + electronRate.max) / 2) + (Math.random() - 0.5) * (electronRate.max - electronRate.min);
					targetVortex = typeof vortex === "number" ? vortex : ((vortex.min + vortex.max) / 2) + (Math.random() - 0.5) * (vortex.max - vortex.min);
					targetBreathAmp = typeof breathAmp === "number" ? breathAmp : ((breathAmp.min + breathAmp.max) / 2) + (Math.random() - 0.5) * (breathAmp.max - breathAmp.min);
		...
		// ── Per-state targets ───────────────────────────────────────────────────
		if (state === "startup") {
			if (startupTimer < 5) assignTargets({ radius: 28, speed: 0.01, bright: 0.5, size: 0.1, lineAmount: 0, electronRate: 0, vortex: 0, breathAmp: 0 }, true);
			startupTimer += dt;
			if (startupTimer >= 5 && startupTimer < 15) {
				assignTargets({ radius: 28, speed: 0.01, bright: 1, size: 0.1, lineAmount: 0, electronRate: 0, vortex: 0, breathAmp: 0 }, true);
			}
			if (startupTimer >= 15 && !pulseSent && startupTimer < 20) {
				assignTargets({ radius: 28, speed: 0.05, bright: 0.3, size: 0.1, lineAmount: 1, electronRate: 0, vortex: 0.1, breathAmp: 4 }, true);
				shockwave = 1.0;
				pulseSent = true;
			}
			if (pulseSent && startupTimer >= 20 && startupTimer < 30) {
				assignTargets({ radius: 26, speed: { min: 0.1, max: 0.3 }, bright: 0.3, size: 0.1, lineAmount: { min: 0.2, max: 0.8 }, electronRate: { min: 0.1, max: 0.5 }, vortex: { min: 0.01, max: 0.05 }, breathAmp: { min: 0.01, max: 0.06 } }, true);
				pulseSent = false;
			}
			if (startupTimer >= 30) {
				startupTimer = 0;
			...
			switch (state) {
				case "idle":
					assignTargets({ radius: 24, speed: { min: 0.1, max: 0.3 }, bright: { min: 0.24, max: 0.26 }, size: 0.1, lineAmount: { min: 0.2, max: 0.8 }, electronRate: { min: 0.1, max: 0.5 }, vortex: { min: 0.01, max: 0.05 }, breathAmp: { min: 0.01, max: 0.06 } });
					break;

				case "listening":
					assignTargets({ radius: 22, speed: { min: 0.3, max: 0.5 }, bright: { min: 0.60, max: 0.70 }, size: 0.1, lineAmount: { min: 0.8, max: 1.4 }, electronRate: { min: 0.3, max: 0.7 }, vortex: { min: 0.1, max: 0.3 }, breathAmp: { min: 0.4, max: 0.6 } });
					break;

				case "thinking":
					assignTargets({ radius: 18, speed: { min: 0.4, max: 0.6 }, bright: 0.7, size: 0.1, lineAmount: 1.0, electronRate: 0.015, vortex: { min: 0.5, max: 0.7 }, breathAmp: { min: 0.1, max: 0.2 } });
					break;

				case "speaking":
					// Speaking: orb more compact + flatter ring silhouette
					assignTargets({ radius: 10, speed: 0.45, bright: 0.78, size: 0.1, lineAmount: 0.92, electronRate: 0, vortex: { min: 1.2, max: 1.6 }, breathAmp: { min: 0.8, max: 1.2 } });
		...
		for (let i = 0; i < N; i++) {
			...
			ParticleVelocities[i3] += Math.sin(t * 0.02 + px * 2.1 + y * 0.1) * 0.0008 * currentSpeed;
			ParticleVelocities[i3 + 1] += Math.cos(t * 0.025 + px * 1.7 + z * 0.1) * 0.0008 * currentSpeed;
			ParticleVelocities[i3 + 2] += Math.sin(t * 0.022 + px * 0.9 + x * 0.1) * 0.0008 * currentSpeed;
			...
			if (speaking) {
				...
				// 2. SHOCKWAVE
				if (shockwave > 0.005) {
					ParticleVelocities[i3] += (x / dist) * shockwave * 0.10;
					ParticleVelocities[i3 + 1] += (y / dist) * shockwave * 0.05;
					ParticleVelocities[i3 + 2] += (z / dist) * shockwave * 0.10;
				}
		...
		if (speaking) {
			...
			ParticleMaterial.size = currentSize + bass * 0.20 + shockwave * 0.30;
