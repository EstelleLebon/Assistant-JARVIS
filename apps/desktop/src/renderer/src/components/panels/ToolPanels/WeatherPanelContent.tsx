    ...
    return (
        ...
        >
            <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                ...
                <div style={{ fontSize: '42px', fontWeight: 700, marginTop: '8px' }}>{w.temp}°</div>
            ...
            >
                {w.feels_like !== undefined && <Stat label="Ressenti" value={`${w.feels_like}°`} />}
                {w.humidity !== undefined && <Stat label="Humidité" value={`${w.humidity}%`} />}
                {w.wind !== undefined && <Stat label="Vent" value={`${w.wind} km/h`} />}
            </div>
        </div>
    )
}
