...
const DEPS = ['xdg-open', 'xdotool', 'wmctrl', 'playerctl', 'notify-send', 'scrot', 'pactl'] as const
...
fastify.get('/health', async () => {
    const results = await Promise.all(DEPS.map((d) => checkDep(d).then((status) => [d, { status }] as [Dep, { status: string }])))
